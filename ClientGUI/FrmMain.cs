...
namespace ClientGUI
{
    public partial class FrmMain : Form
    {
        ...
        private async void button1_Click(object sender, EventArgs e)
        {
            ...
            if (Uri.TryCreate(textBox1.Text, UriKind.Absolute, out result))
            {
        ...
        {
            ChannelFactory<ICompute> channel = new ChannelFactory<ICompute>(endpoint);
            ICompute proxy = channel.CreateChannel();
            var result = proxy.Run(tasks, processors, runtime, timeOut);
            return result;
        }
